...
public class prQuadtree<T extends Compare2D<? super T>> {
	...
	class rectangle {
		 ...
		 *            the maximum y
		 */
		private rectangle(long xMin, long xMax, long yMin, long yMax) {
			x1 = xMin;
			x2 = xMax;
			y1 = yMin;
			y2 = yMax;
	...
	/**
	 * insert a element into the quad-tree
	...
	private prQuadNode recInsert(prQuadNode node, T elem, rectangle rec) {
		...
		if (node.isLeaf()) {
			...
			internal = recInsert(internal, ((prQuadLeaf) node).getElement(),
					rec);
	...
	/**
	 * delete a element from the quad-tree
	 * Pre: elem != null 
	...
	private prQuadNode recDelete(prQuadNode node, T elem, rectangle rec) {
		...
		if (node.isLeaf()) {
			...
			if (((prQuadLeaf) node).getElement().equals(elem)) {
		...
		// child leaf-node and no internal-node, then contract the tree from bottom up
	...
	/**
	 * find a element within quad-tree
	 * Pre: elem != null
	 * 
	 * @param Elem
	...
	/**
	 * find all elements within quad-tree that's within a defined region
	...
	public Vector<T> find(long xLo, long xHi, long yLo, long yHi) {
		// calls the recursive method to look for all points within the defined region
	...
	private Vector<T> recFindAll(prQuadNode node, rectangle region,
		...
		if (node.isLeaf()) {
			...
			if (((prQuadLeaf) node).getElement().inBox(region.x1, region.x2,
					region.y1, region.y2)) {
				result.add(((prQuadLeaf) node).getElement());
			}
